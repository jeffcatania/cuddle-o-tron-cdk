...
export interface EmailReminderLambdaConstructProps {
  ...
  export class EmailReminderLambdaConstruct extends Construct {
    constructor(scope: Construct, id: string, props: EmailReminderLambdaConstructProps = {}) {
      super(scope, id);

      const lambdaRole = new iam.Role(this, 'LambdaRole', {
        assumedBy: new iam.ServicePrincipal('lambda.amazonaws.com'),
        description: 'Execution role for Lambda function to allow access to SES, SNS, State Machine, and Cloudwatch',
        inlinePolicies: {
